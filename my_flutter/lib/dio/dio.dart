...
class Dio {
  ...
  Future<Response<T>> _makeRequest<T>(
    ...
    try {
      ...
      Log.log("DIO responseBody ", responseBody);
  ...
  Future<Stream<List<int>>> _transformData(RequestOptions options,
    ...
    Log.log("DIO", options.method);
    if (data != null && ["POST", "PUT", "PATCH"].contains(options.method)) {
      // Handle the FormData
      int length;
      if (data is Stream) {
        Log.log("DIO", 'data is Stream');
