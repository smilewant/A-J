...
class DefaultHttpClientAdapter extends HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    if (options.connectTimeout > 0) {
      ...
      Log.log("DIO http method", options.method);
      Log.log("DIO http uri", options.uri);
      requestFuture = _httpClient.openUrl(options.method, options.uri);
      requestFuture.then((response){
        Log.log("DIO http response", response);
      });
    }
    ...
    Log.log("DIO http responseStream", responseStream.reasonPhrase);
    Log.log("DIO http responseStream", responseStream.contentLength);
    return ResponseBody(
      responseStream,
